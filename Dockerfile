...
COPY --chown=node:node --from=cloudinary $GHOST_INSTALL/node_modules/ghost-storage-cloudinary-v2 \
  $GHOST_INSTALL/content/adapters/storage/ghost-storage-cloudinary-v2
...
RUN set -ex; \
  ...
  su-exec node ghost config theme "ruby"
